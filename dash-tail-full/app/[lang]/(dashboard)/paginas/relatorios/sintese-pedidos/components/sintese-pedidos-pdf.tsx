...
interface SintesePedidosPDFProps {
  items: RelatorioItem[];
  totais: Totais;
  filtros: Filtros;
}
  ...
  return (
    <View style={styles.headerContainer} fixed>
      ...
      <View style={styles.headerTextContainer}>
        <Text style={styles.title}>Relatório De Síntese de Pedidos</Text>
...
const HeaderRow = memo(() => (
  <View style={styles.headerRow} fixed>
    ...
    <Text style={styles.cellFaturamento}>Faturamento</Text>
  </View>
));
...
const ItemRow = memo(({ item }: { item: RelatorioItem }) => (
  <View style={styles.row}>
    ...
    <Text style={styles.cellNumero}>{item.numero}</Text>
    <Text style={styles.cellValor}>{formatMoney(item.pedido)}</Text>
    <Text style={styles.cellFaturamento}>{formatMoney(item.faturamento)}</Text>
  </View>
));
...
const TotalRow = memo(({ totais }: { totais: Totais }) => (
  <View style={[styles.row, styles.bold]}>
    ...
    <Text style={styles.cellFaturamento}>{formatMoney(totais.faturamento)}</Text>
  </View>
));
...
export const SintesePedidosPDF = memo(({ items, totais, filtros }: SintesePedidosPDFProps) => {
  return (
    <Document>
...
SintesePedidosPDF.displayName = 'SintesePedidosPDF';
