...
import { ComissaoPedidosPDF } from './components/comissao-pedidos-pdf';
...
export function ComissaoPedidosContent() {
  ...
  const [data, setData] = useState<PedidoComissao[]>([]);
  ...
  const mutation = useMutation({
    mutationFn: async (payload: any) => {
      const response = await fetch('/api/paginas/relatorios/comissao', {
        method: 'POST',
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>Relatório de Comissão por Pedidos</CardTitle>
        </CardHeader>
        ...
        <Card>
          <CardHeader>
            <div className="flex justify-between items-center">
              <CardTitle>Resultados - Comissões por Pedido</CardTitle>
              {showPDF && (
                <BlobProvider
                  document={
                    <ComissaoPedidosPDF
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (url) {
                          ...
                          link.href = url;
                          link.download = `comissao-pedidos-${format(new Date(), 'yyyy-MM-dd-HHmmss')}.pdf`;
                          link.click();
                        }
                      }}
                    >
